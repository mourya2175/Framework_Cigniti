...
public class ActionEngine extends TestEngine {
	...
	public boolean type(By locator, String testdata, String locatorName)
		...
		try {
			...
			if (flag) {
				SuccessReport("Type ","Data typing action is performed on \""+locatorName+"\" with data \""+testdata+"\"");
			} else{
				failureReport("Type ","Data typing action is not perform on \""+locatorName+"\" with data \""+testdata+"\"");
			}
		}
		return flag;
	}
	...
	/**
	 * Moves the mouse to the middle of the element and click on it. The element is scrolled
	 * into view and its location is calculated using getBoundingClientRect.
	 * 
	 * @param locator
	 *            : Action to be performed on element (Get it from Object
	 ...
	 * 
	 */
	public boolean mouseoverAndClick(By locator, String locatorName)
			throws Throwable {
		boolean flag = false;
		startStepTime= System.currentTimeMillis();
		try {
	...
	public boolean waitForElementPresent(By by, String locator)
		...
		for (int i = 0; i < 3; i++) {
			try{
				if (driver.findElement(by).isDisplayed()){
					SuccessReport("WaitForElementPresent ","Successfully located the element \""+locator+"\"");   
					flag= true;   
					return flag;
				}
			}catch(Exception e){                     
	 ...
	 * @return Whether or not the element is displayed
	 */
	public boolean waitForElementNotPresent(By by, String locator)
			throws Throwable {
		boolean flag = false; 
		startStepTime= System.currentTimeMillis();
		for (int i = 0; i < 3; i++) {
			try{
				if (!driver.findElement(by).isDisplayed()){
					SuccessReport("WaitForElementNotPresent ","Successfully waited till the element \""+locator+"\" not visible");   
					flag= true;   
					return flag;
				}
			}catch(Exception e){                     
				System.out.println(""+locator+" Element still exists");
				if(i==3)
					failureReport("WaitForElementPresent ","Falied to wait till the element \""+locator+"\" not visible");                       
	...
	public boolean selectBySendkeys(By locator, String value,
		...
		try {
			...
			if (flag) {
				...
				failureReport("Select ","\""+value+"\"is Not Selected from the DropDown\""+locatorName+"\"");
	...
	public String getText(By locator, String locatorName)
		...
		try {
			...
			if (flag) {
				SuccessReport("GetText ", " Able to get Text from \""+locatorName+"\" and text is "+text+"");			
			} else {
	...
	public boolean JSClick(By locator, String locatorName)
		...
		try {
			...
			if (flag) {
				SuccessReport("Click "," Click Action is performed on \""+locatorName+"\"");			
			} else {
				failureReport("Click "," Click Action is not performed on \""+locatorName+"\"");	
			}
		}
		return flag;
	}
